...
# Si utilizamos un incremento negativo, el array se muestra en orden inverso       # El problema es que no muestra el valor 0
...
#  Para  elegir SIMULTANEAMENTE  ambos  elementos,  lo hacemos  utilizando dos  punto  s
print('b[:,0,0]  =',  b[:,0,0])
...
# Si queremos la fila 1 en el bloque 0 (sin que importen las columnas), se tie ne:
print('b[0,1]  =',  b[0,1])
...
# Podemos  observar lo siguiente: entre corchetes encontramos tres valores  # El primero, el cero, selecciona el primer bloque
# El tercero, -1, se encarga de seleccionar la última columna
# Los dos  puntos,  en  la  segunda  posición,  SELECCIONAN  todos  los  # componentes de lad FILAS, que FORMARÁN PARTE de dicha COLUMNA

# Dado que los dos puntos definen todos los valores de las FILAS en
# una columna específica, si quisieramos que DICHOS VALORES estuvieran # en orden inverso, ejecutaríamos la instrucción

print('b[0,  ::-1,  -1]  =',  b[0,  ::-1,  -1])
# La expresión ::-1 invierte todos los valores que se hubieran seleccionado    # Si en lugar de invertir la columna, quisieramos imprimir sus
...
# A partir  de  la  matriz  que  acaba  de  ser  generada,  vamos  a  mostrar # como se construye la transpuesta de la matriz
...
# Para concluir este primer módulo de numpy, mostraremos que la instrucción  # resize, ejecuta una labor similar a reshape
# La diferencia está en que resize altera la estructura del array
# En cambio reshape crea una copia del original, razón por la cual en    # reshape se debe asignar el resultado a una nueva variable
